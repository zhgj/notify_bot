...
class HttpRequest:
    ...
    def save_iciba_mp3_and_img(self, path_dir, iciba_url):
        try:
            ...
            if res != '':
                iciba_info = res.json()
                filename_zh_cn = iciba_info['dateline'] + '_' + iciba_info['note'].strip('。').strip('！')
                filename_en_us = iciba_info['dateline'] + '_' + iciba_info['content'].strip('。').strip('！')
                # filename = urllib.parse.quote(filename)
                mp3_file_path = path_dir + '\\' + filename_en_us + '.mp3'
                with open(mp3_file_path, 'wb') as f:
                    f.write(requests.get(iciba_info['tts']).content)
                img_file_path = path_dir + '\\' + filename_en_us + '.png'
                with open(img_file_path, 'wb') as f:
                    f.write(requests.get(iciba_info['fenxiang_img']).content)
            return mp3_file_path, img_file_path, filename_zh_cn, filename_en_us
        except Exception as e:
            print('iciba接口异常：{0}'.format(e))
            return '', ''

    # 获取API聊天机器人回复内容(bot2已失效)
    def bot_reply(self, bot_url, bot_url2, msg):
        ...
        if '笑话' in msg:
            ...
            return self.bot1_reply(bot_url, msg) if random.randint(0, 1) else self.bot1_reply(bot_url, msg)
